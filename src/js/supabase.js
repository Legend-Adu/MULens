...
/**
 * Fetch the current Photo-of-the-Week (POTW) configuration stored in Supabase Storage JSON file
 */
export async function fetchPOTWFromSupabase() {
  try {
    // Try to read public URL for POTW JSON and fetch it (avoids requiring service role key on client)
    const { data: urlData, error: urlErr } = await supabase.storage.from(BUCKET_NAME).getPublicUrl('site-config/potw.json');
    if (urlErr || !urlData || !urlData.publicUrl) return null;
    const resp = await fetch(urlData.publicUrl);
    if (!resp.ok) return null;
    const text = await resp.text();
    const potw = JSON.parse(text || 'null');
    return potw;
  } catch (err) {
    console.warn('[Supabase POTW Fetch Error]:', err);
    return null;
  }
}

/**
 * Set POTW via serverless API which uses service role key. Requires x-admin-secret header.
 */
export async function setPOTWOnServer({ memId = null, custom = null, adminSecret = '' } = {}) {
  try {
    const res = await fetch('/api/set-potw', {
      ...
      body: JSON.stringify({ action: 'set', memId, custom })
    });
    ...
    console.error('[Set POTW Exception]:', err);
    return null;
  }
}
